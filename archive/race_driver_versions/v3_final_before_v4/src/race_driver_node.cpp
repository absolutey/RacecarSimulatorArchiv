...
void RaceDriverNode::controlLoop()
{
  ...
  const LocalPath local_path = raceline_planner_.plan(ego_state, track_model, obstacles);
  ...
  const double target_speed = speed_planner_.computeTargetSpeed(ego_state, local_path, obstacles);
