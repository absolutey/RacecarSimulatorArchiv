...
private:
  ...
  // Prevents rapid switching between nearly-equal boundary candidates.
  mutable std::string last_selected_reason_{""};
  mutable double last_selected_cost_{std::numeric_limits<double>::infinity()};
  mutable int last_selected_count_{0};

  double hysteresis_keep_ratio_{1.045};
  int hysteresis_min_count_{2};
