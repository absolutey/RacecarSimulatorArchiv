...
: Node("race_driver_node")
{
  ...
  const bool v5_use_reference =
    this->declare_parameter<bool>("v5_use_reference", false);
  const std::string v5_reference_csv_path =
    this->declare_parameter<std::string>("v5_reference_csv_path", "");

  use_mpc_lite_ =
    this->declare_parameter<bool>("use_mpc_lite", true);
  const int mpc_horizon_steps =
    this->declare_parameter<int>("mpc_horizon_steps", 12);
  const double mpc_dt =
    this->declare_parameter<double>("mpc_dt", 0.05);
  const double mpc_max_steer =
    this->declare_parameter<double>("mpc_max_steer", max_steer);
  const int mpc_steer_samples =
    this->declare_parameter<int>("mpc_steer_samples", 11);
  const double mpc_speed_delta =
    this->declare_parameter<double>("mpc_speed_delta", 0.22);
  const double mpc_path_weight =
    this->declare_parameter<double>("mpc_path_weight", 2.4);
  const double mpc_heading_weight =
    this->declare_parameter<double>("mpc_heading_weight", 1.2);
  const double mpc_steer_weight =
    this->declare_parameter<double>("mpc_steer_weight", 0.18);
  const double mpc_steer_rate_weight =
    this->declare_parameter<double>("mpc_steer_rate_weight", 0.35);
  const double mpc_speed_reward_weight =
    this->declare_parameter<double>("mpc_speed_reward_weight", 0.22);
  const double mpc_clearance_weight =
    this->declare_parameter<double>("mpc_clearance_weight", 1.0);
  const double mpc_clearance_margin =
    this->declare_parameter<double>("mpc_clearance_margin", v5_min_boundary_margin);
  ...
  if (v5_use_reference) {
    const bool loaded = v5_racing_planner_.loadReferenceFromCsv(v5_reference_csv_path);

    if (loaded) {
      RCLCPP_INFO(
        this->get_logger(),
        "V5 reference path loaded: %s",
        v5_reference_csv_path.c_str());
    } else {
      RCLCPP_WARN(
        this->get_logger(),
        "Failed to load V5 reference path: %s. Falling back to centerline seed.",
        v5_reference_csv_path.c_str());
    }
  }
  ...
  mpc_lite_controller_.setParameters(
    mpc_horizon_steps,
    mpc_dt,
    wheelbase,
    mpc_max_steer,
    ...
    mpc_heading_weight,
    mpc_steer_weight,
    mpc_steer_rate_weight,
    mpc_speed_reward_weight,
    mpc_clearance_weight,
    mpc_clearance_margin);
...
void RaceDriverNode::controlLoop()
{
  ...
  if (use_mpc_lite_) {
    control = mpc_lite_controller_.compute(ego_state, local_path, target_speed);

    if (!control.valid) {
      control = controller_.compute(ego_state, local_path, target_speed);
    }
  } else {
    control = controller_.compute(ego_state, local_path, target_speed);
  }
